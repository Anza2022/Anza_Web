...
const GamifiedQuizesPage = () => {
  ...
  const searchLesson = (value: string) => {
    setSearchResults(
      testQuizes.filter((e) => e.title.toLowerCase().includes(value))
    );
  };

  const getAllSubjects = () => {
    let set = new Set(testQuizes.map((e) => e.subjectType));
    return Array.from(set);
  };

  const getSubjectsAll = (SpecificSubject: string) => {
    let papers = testQuizes.filter((e) => e.subjectType == SpecificSubject);
    return papers;
  };
              ...
              {searchResults.length > 0 &&
                ...
                >
                  ...
                  <div className="info-box text-xs flex p-1 font-semibold justify-between text-gray-500 bg-gray-300">
                    <span className="mr-1 p-1 px-2 font-xs font-bold ">
          ...
          <div className="flex w-full h-[75vh] justify-center items-center flex-col">
            ...
            >
              <div className="flex">
                ...
                <div>
                  <p className="font-bold">Oops! No test found. </p>
                </div>
              </div>
            </div>
          </div>
        ...
        {!loading && searchvalue.length < 1 &&
          ...
          getAllSubjects().map((e, k) => (
            ...
            >
              ...
              <p className="text-main mt-4 ml-5 text-main font-bold capitalize pb-0 text-lg md:text-1xl font-bold"
          
               >
              {" "}
                {e[0].toUpperCase()}
                {e.substring(1).toUpperCase()} QUIZES
              </p>
              <p
                className="mr-4 mt-4 md:mr-5 mr-5 font-bold text-main cursor-pointer"
             
                onClick={() => {
                  setSelectedSubject(e);
                  router.push("/dashboard/quizes/view_subject");
                }}
              >
                View More
              </p>
...
export default GamifiedQuizesPage;
