...
const AdminLessonPlansPage = () => {
  ...
  async function getLessonPlans() {
    ...
    try {
      let plans = await LessonPlanRepo.getLessonPlans();
    ...
    <AdminDashboardLayout>
      <div className="flex-1 flex flex-col bg-gray-200 dark:bg-darksec  md:ml-52 relative h-screen py-16 w-full overflow-x-hidden ">
        <div className="flex h-12 items-center justify-center bg-white  dark:bg-darkmain pl-5  md:pl-10 w-full">
          <div className="flex text-main font-black text-2xl">LESSON PLANS</div>
        </div>
        {loading ? (
...
<div className="bg-teal-100 border-t-4 border-teal-500 rounded-b text-teal-900 px-4 py-3 shadow-md" role="alert">
  <div className="flex">
    ...
    <div>
      <p className="font-bold">Oops! No LESSON PLANS available. </p>
    </div>
  </div>
</div>
</div>
              ...
              {lessonPlans.map((e, i) => (
                <tr
                  ...
                  onDoubleClick={() => {
                    ...
                    router.push("/admin/lesson_plans/update_lesson_plan");
                  }}
                >
                  {" "}
                  ...
                  {/* <td className="w-44 ">{e.chapter}</td> */}
                  <td className="w-44 ">{e.topic}</td>
        ...
        <div
          onClick={() => router.push("/admin/lesson_plans/add_lesson_plan")}
